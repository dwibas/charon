...
export function initDb() {
  ...
  const defaults = {
    ...
    allow_llm_tp_sl: process.env.ALLOW_LLM_TP_SL || 'false',
    same_mint_cooldown_ms: process.env.SAME_MINT_COOLDOWN_MS || String(60 * 60 * 1000),
  ...
  stratInsert.run('fee_tvl_dump', 'Fee/TVL Spot Dump', 0, JSON.stringify({
    entry_mode: 'spot_on_dump',
    min_source_count: 1,
    require_fee_claim: false,
    fee_tvl_dump_enabled: true,
    min_fee_tvl_24h: 0.20,
    token_age_min_ms: 12 * 60 * 60 * 1000,
    token_age_max_ms: 48 * 60 * 60 * 1000,
